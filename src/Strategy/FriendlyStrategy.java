...
public class FriendlyStrategy implements Strategy, Runnable {
	...
	public void execute(MapModel mapModel, CharacterModel charModel) {
		...
		if (mapModel.treasurePresent == true) {
			...
			friendlyPath = PathValidatorController.friendlyPath(1, mapModel.getMapWidth(), mapModel.getMapHeight(),
					(int) charModel.getCharLocation().getWidth(), (int) charModel.getCharLocation().getHeight(),
					(int) mapModel.getTreasures().get(key).getWidth(), (int) mapModel.treasures.get(key).getHeight(),
					blockedPath);
			...
			friendlyPath = PathValidatorController.friendlyPath(1, mapModel.getMapWidth(), mapModel.getMapHeight(),
					(int) charModel.getCharLocation().getWidth(), (int) charModel.getCharLocation().getHeight(),
					(int) home.getWidth(), (int) home.getHeight(), blockedPath);
			Collections.reverse(friendlyPath);
		}
					...
					if (distance < 3) {
						...
						if (distance != 0) {
							...
							if (distance == 0 && mapModel.treasurePresent == true) {
								...
								if (!charModel.getBackPackItems().contains(key.toString())
										&& charModel.getBackPackCounter() < 10) {
